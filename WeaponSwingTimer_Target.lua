...
function addon_data.target.LoadSettings()
...
function addon_data.target.RestoreDefaults()
    for setting, value in pairs(addon_data.target.default_settings) do
        character_target_settings[setting] = value
    end
...
--[[====================================== LOGIC RELATED =======================================]]--
--[[============================================================================================]]--
function addon_data.target.OnPlayerTargetChanged()
...
function addon_data.target.OnInventoryChange()
...
function addon_data.target.OnUpdate(elapsed)
...
function addon_data.target.OnCombatLogUnfiltered(...)
    local sourceGUID = select(4, ...)
    if (sourceGUID == addon_data.target.guid) then
        ...
        elseif (subevent == "SWING_MISSED") then
            ...
            if destGUID == addon_data.player.guid then
                addon_data.core.MissHandler("target", missType, isOffHand, true)
            end
        elseif (subevent == "SPELL_DAMAGE") or (subevent == "SPELL_MISSED") then
            local spellID = select(12, ...)
            addon_data.core.SpellHandler("target", spellID)
        end
    end

end

function addon_data.target.ResetMainSwingTimer()
    if UnitExists("target") then
        addon_data.target.main_swing_timer = addon_data.target.main_weapon_speed
    end
end

function addon_data.target.ResetOffSwingTimer()
...
function addon_data.target.ZeroizeSwingTimers()
    addon_data.target.main_swing_timer = 0.0001
    addon_data.target.off_swing_timer = 0.0001
end

function addon_data.target.UpdateMainSwingTimer(elapsed)
    if character_target_settings.enabled and UnitExists("target") then
...
function addon_data.target.UpdateOffSwingTimer(elapsed)
    if character_target_settings.enabled and UnitExists("target") then
        if addon_data.target.has_offhand then
...
function addon_data.target.UpdateMainWeaponSpeed()
    if UnitExists("target") then
...
function addon_data.target.UpdateOffWeaponSpeed()
    if UnitExists("target") then
        -- Handle the nil when first selecting target
...
--[[===================================== VISUALS RELATED ======================================]]--
--[[============================================================================================]]--
function addon_data.target.UpdateVisualsOnUpdate()
...
function addon_data.target.UpdateVisualsOnSettingsChange()
    local frame = addon_data.target.frame
    local settings = character_target_settings
...
function addon_data.target.OnFrameDragStart()
    if not character_target_settings.is_locked then
        addon_data.target.frame:StartMoving()
    end
end

function addon_data.target.OnFrameDragStop()
    local frame = addon_data.target.frame
    local settings = character_target_settings
...
function addon_data.target.InitializeVisuals()
    local settings = character_target_settings
    -- Create the frame
...
function addon_data.target.UpdateConfigPanelValues()
...
function addon_data.target.EnabledCheckBoxOnClick(self)
    character_target_settings.enabled = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.ShowOffHandCheckBoxOnClick(self)
    character_target_settings.show_offhand = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.ShowBorderCheckBoxOnClick(self)
    character_target_settings.show_border = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.ClassicBarsCheckBoxOnClick(self)
    character_target_settings.classic_bars = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.FillEmptyCheckBoxOnClick(self)
    character_target_settings.fill_empty = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.ShowLeftTextCheckBoxOnClick(self)
    character_target_settings.show_left_text = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.ShowRightTextCheckBoxOnClick(self)
    character_target_settings.show_right_text = self:GetChecked()
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.WidthEditBoxOnEnter(self)
    character_target_settings.width = tonumber(self:GetText())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.HeightEditBoxOnEnter(self)
    character_target_settings.height = tonumber(self:GetText())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.FontSizeEditBoxOnEnter(self)
    character_target_settings.fontsize = tonumber(self:GetText())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.XOffsetEditBoxOnEnter(self)
    character_target_settings.x_offset = tonumber(self:GetText())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.YOffsetEditBoxOnEnter(self)
    character_target_settings.y_offset = tonumber(self:GetText())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.MainColorPickerOnClick()
...
function addon_data.target.MainTextColorPickerOnClick()
    local colorTable = character_target_settings
    local r = "main_text_r"
    local g = "main_text_g"
    local b = "main_text_b"
...
function addon_data.target.OffColorPickerOnClick()
    local colorTable = character_target_settings
    local r = "off_r"
    local g = "off_g"
    local b = "off_b"
...
function addon_data.target.OffTextColorPickerOnClick()
    local colorTable = character_target_settings
    local r = "off_text_r"
    local g = "off_text_g"
    local b = "off_text_b"
...
function addon_data.target.CombatAlphaOnValChange(self)
    character_target_settings.in_combat_alpha = tonumber(self:GetValue())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.OOCAlphaOnValChange(self)
    character_target_settings.ooc_alpha = tonumber(self:GetValue())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.BackplaneAlphaOnValChange(self)
    character_target_settings.backplane_alpha = tonumber(self:GetValue())
    addon_data.target.UpdateVisualsOnSettingsChange()
end

function addon_data.target.CreateConfigPanel(parent_panel)
